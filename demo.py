...
def demo_activation_analysis(wrapper):
    ...
    for layer, stat in stats.items():
        print(f"  {layer}: mean={stat['mean']:.4f}, std={stat['std']:.4f}, sparsity={stat['sparsity']:.2%}")
...
def demo_attribution(wrapper):
    ...
    print(f"\n各层对最终输出的重要性:")
    for layer, importance in sorted(layer_importance.items(), key=lambda x: x[1], reverse=True)[:5]:
...
def demo_linear_probe(wrapper):
    ...
    X = torch.cat([torch.from_numpy(r).unsqueeze(0) for r in representations], dim=0).numpy()
    y = np.array(labels)
    
    probe_config = ProbeConfig(
        hidden_dim=wrapper.model.config.n_embd,
        num_classes=2,
        probe_type="linear"
    )
...
def demo_representation_analysis(wrapper):
    ...
    for layer_idx in range(wrapper.model.config.n_layer):
        ...
        for text in texts:
            inputs = wrapper.tokenizer(text, return_tensors="pt", padding=True, truncation=True, max_length=32)
    ...
    print(f"\n层间表示余弦相似度:")
    for (l1, l2), sim in sorted(similarity.items(), key=lambda x: x[1], reverse=True)[:10]:
...
def demo_knowledge_edit(wrapper):
    ...
    print("\n1. 应用LoRA适配器...")
    editor.apply_lora(target_layers=["attn.c_attn", "attn.c_proj"])
