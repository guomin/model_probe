...
class Locator:
    ...
    def find_knowledge_location(
        ...
        baseline_outputs = self.model_wrapper.generate(
            inputs["input_ids"], max_new_tokens=5
        )
    ...
    def compute_knowledge_neurons(
        self, prompt: str, target_token: str, top_k: int = 50
    ) -> Dict[str, List[int]]:
        """
        计算与特定知识相关的神经元
        """
    ...
    def attention_based_locate(
        self, input_ids: torch.Tensor, target_pos: int = -1
    ) -> Dict[str, np.ndarray]:
        """
        基于注意力定位关键token
        """
        ...
        return {f"layer_{i}": attn for i, attn in enumerate(attention_weights)}
...
class KnowledgeEditor:
    ...
    def __init__(self, model_wrapper, config: Optional[EditConfig] = None):
        ...
        self.editor_layers: nn.ModuleDict = nn.ModuleDict()
        ...
        for name, module in self.model_wrapper.model.named_modules():
            for target in target_layers:
                if target in name:
                    ...
                    scale_val = self.config.alpha
                    lora_a_module = lora_a
                    lora_b_module = lora_b

                    self.lora_modules.extend([lora_a, lora_b])

                    def lora_forward(x):
                        original_out = original_forward(x)
                        lora_out = lora_b_module(lora_a_module(x)) * scale_val
                        return original_out + lora_out

                    module.forward = lora_forward

        self.model_wrapper.model = self.model_wrapper.model
        print(f"Applied LoRA to {len(self.lora_modules) // 2} layers")
    ...
    def finetune_edit(
        ...
        optimizer = torch.optim.Adam(
            self.lora_modules.parameters()
            if len(self.lora_modules) > 0
            else self.model_wrapper.model.parameters(),
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_start in range(0, len(train_data), self.config.batch_size):
                ...
                texts = [d["prompt"] + " " + d[target_key] for d in batch]
    ...
    def validate_edit(
        self, test_prompts: List[str], expected_outputs: List[str]
    ) -> Dict[str, Any]:
        """
        验证编辑效果
        """
